...
class MockSim:
    def __init__(self):
        ...
        self.experience_data = Memory(state_shape, action_size,
                                      reward_size, max_experience_size)
        s, s_new = np.float32(state), np.float32(state)
        a, r, done = 0, 0, False
        for i in range(max_experience_size):
            self.experience_data.add_data(s, a, s_new, r, done)
...
class TestQnetworkAgent(unittest.TestCase):
    ...
    def test_init(self):
        agent = self.agent
        self.assertEqual(agent.actions_name_list, actions_name_list)
        self.assertEqual(agent.action_indexes, action_indexes)
        self.assertTrue(agent.next_action in actions_name_list)
        expected = 0.99
    ...
    def test_get_next_action(self):
        ...
        self.assertTrue(agent.get_next_action(state) in actions_name_list)
    ...
    def test_get_random_action(self):
        agent = self.agent
        self.assertTrue(agent.get_random_action() in actions_name_list)
    ...
    def test_get_best_action(self):
        agent = self.agent
        self.assertTrue(agent.get_best_action(state) in actions_name_list)
    ...
    def test_learn(self):
        ...
        qout = agent.nn.forward_prop(batch['state'], agent.batch_size)
        expected = max_experience_size-1
        self.assertEqual(expected, len(qout))

        qout_next_state = agent.target_q_model.predict(batch['new_state'],
                                                       agent.batch_size)
        expected = max_experience_size-1
